 ...
 * 3，自适应垃圾回收技术：对任何活的对象，一定能最终追溯到其存活在栈或静态存储区（？）之中的引用。这个引用链条可能会穿过数个对象层次。
 *   因此，如果从栈和静态存储区（？）开始，遍历所有的引用，就能找到所有活的对象。
 ...
 * 3，在类内部，变量定义的先后顺序决定了初始化的顺序，即使散布于方法之间，它们仍然会在任何方法被调用之前被初始化
 *
 *
 *
 *
